...
app.use('/user/:name/', express.static(path.join(__dirname, 'uploadedphotosforslide')));
app.use('/manager/:name/', express.static(path.join(__dirname, 'uploadedphotosforslide')));
const React = require('react');
...
app.use(session({
  secret: ' ', // Replace 'your_secret_key' with a real secret key
  resave: false,
...
app.use(cors({
  origin: 'https://virtualbiochemistry.com',  // Replace with your domain
  methods: ['GET', 'POST'],
...
app.use(express.static('uploadedphotosforslide'));
...
const schemaofreviews = new mongoose.Schema({
  reviewsofadmin: [{
    _id: { type: mongoose.Schema.Types.ObjectId, auto: true },
    headeronrev:String,
    paragraphonrev:String,
...
const Credentialssa3 = mongoose.model("Review",schemaofreviews);
const fs = require('fs');
...
const privateKey2 = fs.readFileSync('/var/www/html/mywebsite/keyofaxios/key99.pem', 'utf8');
const certificate2 = fs.readFileSync('/var/www/html/mywebsite/keyofaxios/cert99.pem', 'utf8');
...
//var merchant_ok_url = `'https://virtualbiochemistry.com/user/${variableofparafterpayment}'?success=true`;
//var merchant_ok_url = "";
 ...
 console.log(city,email,year);
    ...
    if(product || product2){
      res.redirect('/emailexist.html');
          ...
          res.cookie('userLoggedData', JSON.stringify(payloada), { maxAge: 900000, httpOnly: false,secure:true });
...
app.get('/user/:name', async (req, res) => {
  ...
  //console.log('www',putonthedb);
   ...
   console.log('pulldernameandsurname2',pulldernameandsurname2);
...
app.post('/paymentinterface', async(req,res)=>{
 ...
 console.log('token',token);
  ...
  console.log('UPON THE REQ');
  request(options, function (error, response, body) {
    console.log('upon error of req');
    if (error) throw new Error(error);
    var res_data = JSON.parse(body);
    console.log('below res_data');  
...
app.post('/user/:name/routeofenquiry',upload2.any(), async (req,res,next) =>{
 ...
 console.log('texttodb',texttodb);
 if(texttodb){
  usingcookie4.enquiryy.push(texttodb);
  console.log('usingcookie4 pushed');
 } 
...
console.log(takenfilenameforenquiry3);
  ...
  console.log('usingcookie4 pushed222');
}else{
...
if(usingcookie4.enquiryisnotreaded === false){
  ...
  console.log('enquiryisnotreaded is true');
}
...
const storageforslide11 = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'uploadedphotosforslide/'); // Set the destination folder for uploads
  },
...
const upload11 = multer({ storage: storageforslide11 });


app.post('/display-reviews-uploadphoto',upload11.single('image'), async (req,res) =>{
  console.log('display-reviews-uploadphoto is ran');
  if (req.file) {
    var imageUrl22 = `${req.file.filename}`;
    
    res.status(200).json({ imageUrl22: imageUrl22 });
...
app.post('/check-admin', async(req,res)=>{
  var admincookie = req.cookies.userLoggedData; 
  console.log(admincookie);
  if(admincookie){
    var adminParsedCookie = JSON.parse(admincookie);
  console.log('this is admincookie', adminParsedCookie.user_is_admin);
  if(adminParsedCookie.user_is_admin==='adminyes'){
    res.send('admin');
  }else{
    res.send('nosomexxxthingelse');
  }
  }else{
...
app.post('/save-slide', async (req, res) => {
  const { slideIndex, newHeader, newParagraph } = req.body;
  console.log(slideIndex, newHeader, newParagraph);

  const filePath = path.join(__dirname, '/views/newformat.html');
  let reviewId;
  if (newHeader.trim().length > 0 && newParagraph.trim().length > 0) {
    try {
    
      let xqw = new Credentialssa3({
        reviewsofadmin: [{
          headeronrev: newHeader,
          paragraphonrev: newParagraph,
      ...
      return res.status(500).send('Error saving review to MongoDB');
    }
  }
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    const document = dom.window.document;
     ...
     const slide = document.querySelectorAll('.mySlides')[slideIndex];
    if (slide) {      
      const header123 = slide.querySelector('.paragraphonslideimg');
      const paragraph123 = slide.querySelector('.paragraphonslideimg2');
      slide.setAttribute('data-review-id', reviewId);
      if (header123) header123.textContent = newHeader;
      if (paragraph123) paragraph123.textContent = newParagraph;
      ...
      fs.writeFile(filePath, updatedHtml, 'utf8', err => {
        ...
        res.send('Slide updated successfully22222222222222');
      });
    } else {
...
app.post('/like', async (req, res) => {
  const { reviewId , minusit , increaseit} = req.body;

 
    console.log('Review ID:', reviewId);
  
    const document = await Credentialssa3.findById(reviewId);
    ...
    if(increaseit){
    const review = document.reviewsofadmin[0]; 
    if (review) {
      review.likes += 1;
      await document.save(); 
      
      broadcastUpdate(reviewId, review.likes, review.dislikes);
    } else {
...
function broadcastUpdate(reviewId, likes, dislikes) {
  console.log('here worked');
  wss.clients.forEach((client) => {
    if (client.readyState === WebSocket.OPEN) {
      
      client.send(JSON.stringify({ reviewId, likes, dislikes, message:'forbuttonsoflike' }));
...
app.post('/get-review-counts', async (req, res) => {
  console.log('in the get-review-counts');
...
const storageforslide = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, 'uploadedphotosforslide/');  
  },
  filename: function (req, file, cb) {
...
const upload2233 = multer({ storage: storageforslide });
let imageUrl;
app.post('/upload-image', upload2233.single('image'), (req, res) => {
  if (req.file) {
...
function changePhotoOfSlide(slideIndex){
  const filePath = path.join(__dirname, '/views/newformat.html');
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    const slide = document.querySelectorAll('.mySlides')[slideIndex];
      ...
      let selectedImg = slide.querySelector('img');
      selectedImg.src = imageUrl;
      selectedImg.style.verticalAlign  = 'middle';
      selectedImg.style.zIndex = '99999';
      ...
      selectedImg.style.fontWeight = '400';
      selectedImg.style.height = '350px';
      selectedImg.style.lineHeight = '24px';
      selectedImg.style.overflowClipMargin = 'content-box';
      selectedImg.style.overflowX = 'clip';
      selectedImg.style.overflowY = 'clip';
      selectedImg.style.textAlign = 'start';
      selectedImg.style.textSizeAdjust = '100%';
      selectedImg.style.verticalAlign = 'middle';
      selectedImg.style.visibility = 'visible';
      selectedImg.style.width = '1000px';
      selectedImg.style.zIndex = '99999';
      ...
      fs.writeFile(filePath, updatedHtml, 'utf8', err => {
        ...
        console.log('photopersist triggered333333333');
...
app.post('/photo-persist', async (req, res) => {
   
  let slideIndex = req.body.slideIndex;
   
  changePhotoOfSlide(slideIndex);
});


app.get('/display-reviews', async (req, res) => {
  try {
    const reviews = await Credentialssa3.find({});
...
app.delete('/delete-review/:id', async (req, res) => {
  try {
    ...
    await Credentialssa3.updateOne(
      { 'reviewsofadmin._id': reviewId },
      { $pull: { reviewsofadmin: { _id: reviewId } } }
    );
...
app.post('/routeofaskedquestionsonmainpage2', async(req,res)=>{
  ...
  const info = await transporter.sendMail({
    from:'ege_gurel35@hotmail.com',
    to:'egegurel@virtualbiochemistry.com',
...
const io2 = io(httpsServer,{ path: '/socket.io' ,  cors: {
  origin: "https://virtualbiochemistry.com",
  methods: ["GET", "POST"]
}});
